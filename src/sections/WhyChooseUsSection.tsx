      ...
      <div className={PAGE_CONTAINER}>
        ...
        <div className="mt-8 grid grid-cols-1 gap-4 sm:mt-10 sm:grid-cols-2 sm:gap-6 lg:mt-14 lg:gap-8">
          {t.whyChooseUs.cards.map((card) => (
            <article
              key={card.title}
              className="card-elevated flex flex-col gap-4 rounded-3xl bg-surface p-5 sm:flex-row sm:gap-5 sm:p-6 lg:p-8"
            >
              <span
                className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-brand-pink-muted text-xl sm:h-14 sm:w-14 sm:text-2xl"
                aria-hidden
              >
                {card.icon}
              </span>
              <div className="min-w-0">
                <h3 className="font-heading text-lg font-semibold text-foreground sm:text-xl">{card.title}</h3>
