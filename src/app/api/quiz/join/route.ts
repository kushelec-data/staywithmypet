...
export async function POST(request: Request) {
  const body = (await request.json().catch(() => null)) as { pin?: string; displayName?: string } | null;
  ...
  const joined = await registerPlayer(pin, displayName);
  if ("error" in joined) return NextResponse.json({ error: joined.error }, { status: 400 });
  const response = NextResponse.json({ ok: true, gameId: joined.gameId, playerId: joined.playerId, token: joined.token });
