...
import { createQuiz, duplicateQuiz, listQuizzes, startDefaultLiveGame, startLiveGame } from "@/lib/quiz/store";
...
export async function POST(request: Request) {
  ...
  if (body?.startLiveGame) {
    const result = await startDefaultLiveGame(gate.session.userId);
    if ("error" in result) return NextResponse.json({ error: result.error }, { status: 400 });
    return NextResponse.json(result);
  }
  if (body?.startQuizId) {
