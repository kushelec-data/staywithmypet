...
export function MembershipUpsellToast({
  ...
  return createPortal(
    <div
      className="pointer-events-none fixed inset-x-0 bottom-0 z-[60] flex justify-center px-3 pb-[max(1rem,env(safe-area-inset-bottom))] sm:inset-x-auto sm:right-4 sm:left-auto sm:bottom-6 sm:justify-end sm:px-0"
      role="region"
      aria-live="polite"
      aria-label={title}
    >
      <div className="membership-upsell-toast pointer-events-auto w-full max-w-sm rounded-2xl border border-brand-teal/30 bg-[#fffaf2]/95 p-4 shadow-[0_14px_44px_rgba(15,60,55,0.14)] backdrop-blur-md dark:border-brand-teal/25 dark:bg-surface/95">
        <div className="flex items-start gap-3">
          <div className="min-w-0 flex-1">
            <p className="font-heading text-sm font-bold leading-snug text-foreground">{title}</p>
            <p className="mt-1 text-xs leading-relaxed text-muted">{body}</p>
            <div className="mt-3 flex flex-wrap items-center gap-2">
              <Button href={upgradeHref} variant="primary" size="sm" onClick={onClose}>
