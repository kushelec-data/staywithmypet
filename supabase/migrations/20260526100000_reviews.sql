-- Reviews after completed bookings (replaces legacy request-based reviews table).
...
create index reviews_request_id_idx on public.reviews (request_id);
...
create index reviews_review_type_idx on public.reviews (review_type);
...
-- Sync Pet Friend ratings on profiles (parent → friend reviews only).
