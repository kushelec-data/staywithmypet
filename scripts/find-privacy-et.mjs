...
const zipPath = path.join(root, ".find-et.zip");
const extractDir = path.join(root, ".find-et");
...
const needles = ["Küpsis", "küpsis", "7.", "Sarnased", "brauseri"];
for (let r = 1; r <= maxR; r++) {
  for (let c = 2; c <= 5; c++) {
    const t = getCell(rows, r, c);
    if (!t) continue;
    if (needles.some((n) => t.includes(n))) {
      console.log(`R${r} C${c}: ${t.slice(0, 100)}`);
    }
  }
}
