...
const zipPath = path.join(root, ".terms-debug2.zip");
const extractDir = path.join(root, ".terms-debug2");
...
const needle = "Lareflexion";
for (let r = 1; r <= maxR; r++) {
  const c2 = getCell(rows, r, 2);
  if (c2.includes(needle)) console.log(`R${r}: ${c2}`);
}
