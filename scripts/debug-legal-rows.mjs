...
for (const name of ["privacy policy", "Safety guidelines", "Terms of Use", "Our Story"]) {
  const rows = parseSheet(
    fs.readFileSync(path.join(extractDir, `xl/worksheets/sheet${idx[name]}.xml`), "utf8"),
    strings,
  );
  console.log(`\n=== ${name} ===`);
  ...
  for (let r = 1; r <= Math.min(maxR, 30); r++) {
    ...
    for (let c = 2; c <= 5; c++) {
      const v = rows[r][c];
      if (v) parts.push(`C${c}:${v.slice(0, 60)}`);
    }
