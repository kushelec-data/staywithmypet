/**
 * Applies newsletter_subscribers migration.
...
async function main() {
  ...
  const dbUrls = resolveDbUrls(env);
  if (!dbUrls) {
    console.error(
  ...
  const sqlPath = path.join(
    __dirname,
    "..",
    "supabase/migrations/20260720140000_newsletter_subscribers.sql",
  );
  ...
  for (const dbUrl of dbUrls) {
    ...
    try {
      ...
      console.log("Applied: supabase/migrations/20260720140000_newsletter_subscribers.sql");
